...
batch_inference_body = MultivariateBatchDetectionOptions(
    ...
    start_time=datetime.strptime("2023-03-21T00:00:00Z", time_format),
    end_time=datetime.strptime("2023-06-05T00:00:00Z", time_format),
)

# get most recent model from model_id.txt
with open('model_id.txt', 'r') as f:
    model_id = f.read().strip()
print('Detection model id is', model_id)
...
if result_status == 'FAILED':
    ...
    y1_scale = 1.5
    if df_melted['value'].min() < 0.1:
        y1_min = -0.5
    else:
        y1_min = df_melted['value'].min() * y1_scale
    if df_melted['value'].max() <= 1:
        y1_max = 1.5
    else:
